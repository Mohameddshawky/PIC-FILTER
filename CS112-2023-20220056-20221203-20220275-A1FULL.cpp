...
// Author3: Youssef Mohamed Hussin , Id: 20221203,section_number=s11
...
void skew_image_horizontally(){
    ...
    for (int i = 0; i < SIZE ; ++i) {
        for (int j = 0; j*cmp < SIZE ; ++j) {
            val[i][j] = image[i][(int)ceil(j*cmp)];//compress the image
        }
    }
    unsigned char tmp[SIZE][SIZE];//decleare a new image
...
void skew_image_vertically(){
    ...
    for (int i = 0; i < SIZE ; ++i) {
        for (int j = 0; j*cmp < SIZE ; ++j) {
            val[j][i] = image[(int)ceil(j*cmp)][i];//compress the image
        }
    }
    unsigned char tmp[SIZE][SIZE];//decleare a new image
    memset(tmp,255, sizeof(tmp));//make tmp white image
